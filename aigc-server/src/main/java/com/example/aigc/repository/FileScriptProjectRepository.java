...
public class FileScriptProjectRepository implements ScriptProjectRepository {
    ...
    @Override
    public List<ScriptProjectSummary> findAll() {
        return readSummaries();
    }

    @Override
    public synchronized void delete(String projectId) {
        storageSupport.deleteRecursively(resolveProjectDir(projectId));
        List<ScriptProjectSummary> filtered = readSummaries().stream()
                .filter(item -> item.projectId == null || !item.projectId.equals(projectId))
                .toList();
        storageSupport.writeValue(INDEX_FILE, filtered);
    }
