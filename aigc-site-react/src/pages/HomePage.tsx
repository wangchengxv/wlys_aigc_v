...
const ONELINK_PROMO_SEEN_KEY = 'onelinkPromoSeen'
...
export function HomePage() {
  ...
  useEffect(() => {
    try {
      if (window.localStorage.getItem(ONELINK_PROMO_SEEN_KEY)) return
      window.localStorage.setItem(ONELINK_PROMO_SEEN_KEY, '1')
      navigate('/models?promo=onelink', { replace: true })
    } catch {
      navigate('/models?promo=onelink', { replace: true })
    }
  }, [navigate])
