...
export function ScriptProjectCreatePage() {
  ...
  const [styleInputMode, setStyleInputMode] = useState<'preset' | 'custom'>(() =>
    globalVisualStyleMode === 'preset' && !!globalVisualStylePresetId ? 'preset' : 'custom',
  )
  const [selectedPresetId, setSelectedPresetId] = useState(() => globalVisualStylePresetId ?? 'film-cinematic')
  const [customVisualStyle, setCustomVisualStyle] = useState(() =>
    globalVisualStyleMode === 'custom'
      ? globalCustomVisualStyle
      : resolveVisualStyleForProject(useGlobalSettingsStore.getState()),
  )
  ...
  const presetGroups = useMemo(() => groupPresetsByCategory(), [])
    ...
    try {
      ...
      const visualStyle =
        styleInputMode === 'preset'
          ...
          : customVisualStyle.trim() || resolveVisualStyleForProject(useGlobalSettingsStore.getState())
      const base = {
        name,
        visualStyle,
        aspectRatio,
        ...
        <div className="grid">
          ...
          <label className="model-field">
            ...
            <span className="hint muted">
              {styleInputMode === 'preset'
                ? `将提交为 preset:风格库 id，后端会映射为词典锚点；当前：${selectedPreset ? presetDescriptor(selectedPreset) : '未选择'}`
                : '可填写自由中文/英文描述（不必是固定 key）；留空则沿用全局设定中的风格（含长文本模式整段提示词）。'}
            </span>
          </label>
          <label className="model-field">
