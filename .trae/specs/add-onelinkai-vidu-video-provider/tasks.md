...
- [x] Task 1: 在 catalog.go 中注册 vidu_onelink Provider
  ...
  - [x] AuthMode 为 `AuthBearer`，VideoSubmitPath 为 `/vidu//vidu/vidu/ent/v2/img2video`，VideoResultPath 为 `/vidu/ent/v2/tasks/{taskId}/creations`
...
- [x] Task 3: 扩展预置模型列表
  ...
  - [x] 覆盖 `viduq3-turbo`、`video-viduq3-pro`、`image-vidu-q2-fast`、`image-vidu-q2`、`viduq2-turbo`、`viduq2`、`viduq1`、`viduq1-classic`、`vidu2.0` 等模型
  - [x] Provider 设为 `vidu_onelink`，BaseURL 设为 `https://api.onelinkai.cloud`

- [x] Task 4: 验证与回归测试
  - [x] 确认 OneLinkAI Vidu 模型可被 `resolveModel("video", ...)` 识别
  - [x] 确认 OneLinkAI Vidu 路由到正确路径 `/vidu//vidu/vidu/ent/v2/img2video`
